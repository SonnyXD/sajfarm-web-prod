...
<html>
    ...
    <span style="font-weight: bold;">{{$institution->name}}</span>
        ...
        <h2 style="font-weight:bold; text-align: center;">TRANSFER</h2>
        <br>
        <span style="">Numar document: {{$transfer->id}} / {{$new_date}}</span>
        <br>
        <span style="">Gestiune de iesire: {{\App\Models\Inventory::where('id', $transfer->from_inventory_id)->first()->name}}</span>
        <br>
        <span style="">Gestiune de intrare: {{\App\Models\Inventory::where('id', $transfer->to_inventory_id)->first()->name}}</span>
        <br>
        <br>
        <br>
            <table border="1">
